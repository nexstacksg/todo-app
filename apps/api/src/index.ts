...
app.use("/*", cors({
  origin: ["https://todo.singaporetestlab.com", "http://localhost:3000"],
...
app.patch("/todos/:id", async (c) => {
  ...
  const result = await query("UPDATE todos SET completed = $1, updated_at = NOW() WHERE id = $2 RETURNING *", [completed, id])
